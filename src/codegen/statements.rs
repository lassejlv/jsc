...
impl CodeGen {
    pub(crate) fn emit_statement(&mut self, stmt: &Statement<'_>) {
        ...
        match stmt {
            ...
            // Classes
            Statement::ClassDeclaration(cls) => self.emit_class_decl(cls),
    ...
    pub(crate) fn emit_var_decl(&mut self, decl: &VariableDeclaration<'_>) {
        for declarator in &decl.declarations {
            let init_val = if let Some(init) = &declarator.init {
                // Track class expression → variable name aliases
                if let Expression::ClassExpression(cls) = init {
                    if let BindingPattern::BindingIdentifier(var_id) = &declarator.id {
                        let var_name = var_id.name.as_str().to_string();
                        // The class_info key is the class id name or the anonymous name
                        // that was just generated by emit_class_expr (next_anon_fn - 1 after call)
                        let class_key = cls
                            .id
                            .as_ref()
                            .map(|id| id.name.as_str().to_string())
                            .unwrap_or_else(|| format!("__AnonClass{}", self.next_anon_fn));
                        // Emit the expression (this compiles the class and increments next_anon_fn)
                        let val = self.emit_expression(init);
                        // Now resolve the actual key (may have been anon)
                        let actual_key = cls
                            .id
                            .as_ref()
                            .map(|id| id.name.as_str().to_string())
                            .unwrap_or_else(|| format!("__AnonClass{}", self.next_anon_fn - 1));
                        // Clone the class_info entry under the variable name
                        if let Some(info) = self.class_info.get(&actual_key) {
                            let cloned = super::classes::ClassInfo {
                                constructor_fn: info.constructor_fn.clone(),
                                prop_init_fn: info.prop_init_fn.clone(),
                                methods: info.methods.clone(),
                                static_methods: info.static_methods.clone(),
                                getters: info.getters.clone(),
                                setters: info.setters.clone(),
                                super_class: info.super_class.clone(),
                            };
                            self.class_info.insert(var_name.clone(), cloned);
                            self.var_class_map.insert(var_name, actual_key);
                        }
                        self.emit_binding_pattern(&declarator.id, &val);
                        continue;
                    }
                }
    ...
    pub(crate) fn emit_binding_pattern(&mut self, pattern: &BindingPattern, value_reg: &str) {
        match pattern {
            ...
            BindingPattern::ObjectPattern(op) => {
                // Collect destructured key names for rest exclusion
                let mut destructured_keys: Vec<String> = Vec::new();
                for prop in &op.properties {
                    ...
                    destructured_keys.push(key_name.clone());
                ...
                if let Some(rest) = &op.rest {
                    // Build exclusion array from destructured keys
                    let excl_arr = self.fresh_reg();
                    self.emit(&format!("  {} = call i64 @js_array_new()", excl_arr));
                    for key_name in &destructured_keys {
                        let key_str = self.emit_string_const(key_name);
                        self.emit(&format!(
                            "  call i64 @js_array_push_val(i64 {}, i64 {})",
                            excl_arr, key_str
                        ));
                    }
                    let rest_obj = self.fresh_reg();
                    self.emit(&format!(
                        "  {} = call i64 @js_object_rest(i64 {}, i64 {})",
                        rest_obj, value_reg, excl_arr
                    ));
                    self.emit_binding_pattern(&rest.argument, &rest_obj);
                }
            }
            BindingPattern::ArrayPattern(ap) => {
                ...
                if let Some(rest) = &ap.rest {
                    ...
                    let idx_alloca = {
                        ...
                        self.emit(&format!(
                            "  store i64 {}, ptr {}, align 8",
                            js_number_bits(start_idx as f64),
                            m
                        ));
                        m
                    };
    ...
    fn emit_for(&mut self, stmt: &ForStatement<'_>) {
        ...
        self.emit_label(&body_label);
        self.loop_stack
            .push((end_label.clone(), update_label.clone()));
        self.emit_statement(&stmt.body);
        self.loop_stack.pop();
    ...
    fn emit_for_of(&mut self, stmt: &ForOfStatement<'_>) {
        ...
        if is_simple {
            if let BindingPattern::BindingIdentifier(id) = &decl.declarations[0].id {
                ...
                self.emit(&format!("  store i64 {}, ptr {}, align 8", elem, alloca));
            }
        } else {
    ...
    fn emit_try(&mut self, stmt: &TryStatement<'_>) {
        ...
        let after_try = if has_finally {
            &finally_label
        } else {
            &end_label
        };
        let after_catch = if has_finally {
            &finally_label
        } else {
            &end_label
        };
        ...
        self.emit(&format!("  {} = icmp ne i32 {}, 0", is_catch, setjmp_ret));
    ...
    fn emit_for_in(&mut self, stmt: &ForInStatement<'_>) {
        ...
        let var_name = match &stmt.left {
            ForStatementLeft::VariableDeclaration(decl) => match &decl.declarations[0].id {
    ...
    fn emit_ts_enum(&mut self, decl: &TSEnumDeclaration<'_>) {
        ...
        self.emit(&format!("  store i64 {}, ptr {}, align 8", obj, alloca));
            ...
            for spec in specifiers {
                match spec {
                    ImportDeclarationSpecifier::ImportSpecifier(s) => {
                        ...
                        // Resolve class alias: if the exported name maps to a class, register locally
                        if let Some(class_key) = self
                            ...
                            .cloned()
                        {
                            if let Some(info) = self.class_info.get(&class_key) {
                                let cloned = super::classes::ClassInfo {
                                    constructor_fn: info.constructor_fn.clone(),
                                    prop_init_fn: info.prop_init_fn.clone(),
                                    methods: info.methods.clone(),
                                ...
                                self.class_info.insert(local_name.to_string(), cloned);
                                self.var_class_map.insert(local_name.to_string(), class_key);
                            }
                        }
                    }
                    ...
                    ImportDeclarationSpecifier::ImportNamespaceSpecifier(s) => {
                        ...
                        let local_name = s.local.name.as_str();
                        let alloca = self.declare_var(local_name);
                        self.emit(&format!("  store i64 {}, ptr {}, align 8", exports, alloca));
                    }
                }
            }
        }
    ...
    fn emit_export_named(&mut self, decl: &ExportNamedDeclaration<'_>) {
                    ...
                    if let Some(id) = &func.id {
                        ...
                        if let Some(llvm_name) = self.known_functions.get(name).cloned() {
                            ...
                            for i in 0..arity {
                                writeln!(
                                    tramp,
                                    "  %p{} = getelementptr i64, ptr %args, i32 {}",
                                    i, i
                                )
                                .unwrap();
                                writeln!(tramp, "  %a{} = load i64, ptr %p{}, align 8", i, i)
                                    .unwrap();
        ...
        if !decl.specifiers.is_empty() && decl.source.is_none() {
            ...
            for spec in &decl.specifiers {
                ...
                // Track class exports: if local_name is a known class, record the mapping
                if self.class_info.contains_key(local_name) {
                    self.module_class_exports
                        .entry(mod_id)
                        .or_insert_with(HashMap::new)
                        .insert(exported_name.to_string(), local_name.to_string());
                }
            }
        }
    }
            ...
            ExportDefaultDeclarationKind::FunctionDeclaration(func) => {
                ...
                if func.id.is_some() {
                    ...
                    if let Some(llvm_name) = self.known_functions.get(name).cloned() {
                        ...
                        let mut param_regs = Vec::new();
                        for i in 0..arity {
                            writeln!(tramp, "  %p{} = getelementptr i64, ptr %args, i32 {}", i, i)
                                .unwrap();
